...
from mal_gui.undo_redo_commands.create_goal_connection_command import (
...
class ModelScene(QGraphicsScene):
    ...
    def _start_connection(self, event, item):
        ...
        self.line_item = QGraphicsLineItem(QLineF(event.scenePos(), event.scenePos()))
    ...
    def _finalize_connection(self, released_item):
        ...
        self.end_item = released_item if isinstance(released_item, ItemBase) else None
    ...
    def _create_attacker_connection(self):
        ...
        asset = self.end_item if attacker is self.start_item else self.start_item
    ...
    def _create_goal_connection(self, attacker, asset):
        dialog = GoalConnectionDialog(attacker, asset, self.lang_graph, self.model)
    ...
    def draw_model(self):
        ...
        for asset in self.model.assets.values():
            if "position" in asset.extras:
                pos = QPointF(
                    asset.extras["position"]["x"], asset.extras["position"]["y"]
                )
    ...
    def add_association_connection(self, start_item, end_item, fieldname: str):
        ...
        connection = AssociationConnectionItem(fieldname, start_item, end_item, self)
    ...
    def add_entrypoint_connection(self, attack_step_name, attacker_item, asset_item):
        """Add attacker entrypoints to the scene"""

        connection = EntrypointConnectionItem(
            attack_step_name, attacker_item, asset_item, self
        )
    ...
    def add_goal_connection(self, attack_step_name, attacker_item, asset_item):
    ...
    def recreate_asset(self, asset_item: AssetItem, position: QPointF) -> AssetItem:
    ...
    def create_asset(
        ...
        new_asset_item = self.asset_factory.create_asset_item(new_asset, position)
    ...
    def remove_goal(self, goal_item: GoalConnectionItem):
        ...
        full_name = goal_item.asset_item.asset.name + ":" + goal_item.attack_step_name
    ...
    def decontainerize_assets(self, currently_selected_container: AssetsContainer):
        ...
        for item_entry in currently_selected_container.containerized_assets_list:
            ...
            original_position_of_item = (
                current_position_of_container + item_entry["offset"]
            )
            self.addItem(item)
    ...
    def expand_container(self, currently_selected_container):
        ...
        current_centroid_position_of_container = currently_selected_container.scenePos()
        ...
        rectangle_bounding_all_containerized_assets = (
            self.calc_surrounding_rect_for_grouped_assets_in_container(
                contained_item_for_bounding_rect_calc
            )
        )
    ...
    def serialize_entrypoints(
        ...
        for conn in entrypoints:
            ...
            # If entry points
            both_items_selected = (
                conn.asset_item.asset.id in selected_asset_ids
                and conn.attacker_item.name in selected_attacker_names
            )
    ...
    def serialize_graphics_items(self, items: list[ItemBase], cut_intended):
        ...
        selected_attacker_names = {
            item.name for item in items if isinstance(item, AttackerItem)
        }

        selected_asset_ids = {
            item.asset.id
            for item in items
            if isinstance(item, AssetItem) and item.asset.id is not None
        }
        ...
        base64_serialized_data = base64.b64encode(serialized_data).decode("utf-8")
    ...
    def show_assets_container_box_context_menu(
        ...
        menu = QMenu()
        assets_compress_container_box_action = QAction("Compress Container Box", self)
    ...
    def show_items_details(self):
        ...
        if len(selected_items) == 1:
            ...
            self.main_window.item_details_window.update_item_details_window(None)
            self.main_window.update_properties_window(None)
            self.main_window.update_attack_steps_window(None)
            self.main_window.update_asset_relations_window(None)

    def calc_surrounding_rect_for_grouped_assets_in_container(
        self, contained_item_for_bounding_rect_calc
    ):
