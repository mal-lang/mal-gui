...
class FlowView(QGraphicsView):
    def __init__(self, scene, parent=None):
        ...
        if scene is not None:
            self.setScene(scene)
            self.setAcceptDrops(True) #Akash added for drag/drop
    ...
    #Setup drag and drop of nodes from sidetab
    def dragEnterEvent(self,event):
        if event.mimeData().hasText():
            model_name = event.mimeData().text()
            print("dragEnterEvent - Model Name = " + model_name)
            event.acceptProposedAction()
        
    def dragMoveEvent(self, event: QDragMoveEvent):
        print("dragMoveEvent - Model Name = " + event.mimeData().text())
    ...
    def dropEvent(self,event):
        if event.mimeData().hasText():
            model_name = event.mimeData().text()
            print("dropEvent - Model Name = " + model_name)
            try:
                model, _ = self._scene.registry.get_model_by_name(model_name)
            except ValueError:
                logger.error("Model not found: %s", model_name)
                ...
                node = self._scene.create_node(model)
                pos_view = self.mapToScene(event.position().toPoint())
                node.graphics_object.setPos(pos_view)
                self._scene.node_placed.emit(node)
        event.acceptProposedAction()
